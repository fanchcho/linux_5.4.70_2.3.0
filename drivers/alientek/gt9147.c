...
const u8 irq_table[] = { IRQ_TYPE_EDGE_RISING, IRQ_TYPE_EDGE_FALLING,
			 IRQ_TYPE_LEVEL_LOW,
...
static int gt9147_ts_reset(struct i2c_client *client, struct gt9147_dev *dev)
{
	...
	if (gpio_is_valid(dev->irq_pin)) {
		...
		ret = devm_gpio_request_one(&client->dev, dev->irq_pin,
					    GPIOF_OUT_INIT_HIGH, "gt9147 int");
		if (ret) {
			return ret;
		}
	}
	...
	if (touch_num) { /* 单点触摸按下 */
		...
		if (id == 0) {
			...
			input_mt_report_slot_state(dev->input, MT_TOOL_FINGER,
						   true);
			input_report_abs(dev->input, ABS_MT_POSITION_X,
					 input_x);
			input_report_abs(dev->input, ABS_MT_POSITION_Y,
					 input_y);
		}
				...
				struct gt9147_dev *dev)
{
	...
	printk("X_MAX: %d, Y_MAX: %d, TRIGGER: 0x%02x", dev->max_x, dev->max_y,
	       dev->irqtype);
...
int gt9147_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* 1，获取设备树中的中断和复位引脚 */
	gt9147.irq_pin =
		of_get_named_gpio(client->dev.of_node, "interrupt-gpios", 0);
	gt9147.reset_pin =
	...
	input_set_abs_params(gt9147.input, ABS_MT_POSITION_X, 0, gt9147.max_x,
			     0, 0);
	input_set_abs_params(gt9147.input, ABS_MT_POSITION_Y, 0, gt9147.max_y,
			     0, 0);
